...
public class S3MultipartUploadHandler implements MultipartUploadHandler {
  ...
  public void putPart(InputStream inputStream, Long lengthInBytes, Integer partNumber) {
    try {
      ...
      logger.warn("UploadId {} Part {} uploading to {}", uploadId, partNumber, key);
  ...
  public void complete() {
    ...
    if (partETags.size() == 0) {
      logger.warn("UploadId {} has no parts uploaded, aborting upload", uploadId);
      abort();
    ...
    logger.warn("UploadId {} upload completing partSize {}", uploadId, partETags.size());
